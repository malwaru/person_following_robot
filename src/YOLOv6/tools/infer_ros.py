...
class Inferer:
    ...
    def run (self,input_method):
        print(f"Running with {input_method}")
...
def run_stream(vid_source=0,depth=False):
    ...
    weights=osp.join(ROOT,'YOLOv6/weights/yolov6n.pt')
    device = 'gpu'
    yaml = osp.join(ROOT,'YOLOv6/data/coco.yaml')
    img_size=640
    half=False
...
def main():
    ...
    print("Starting the person recognition")
    run_stream(vid_source=0)
    # test_webcam()
    # test_intelrealsense()
