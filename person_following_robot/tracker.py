...
if __name__ == '__main__':
	...
	print(f"Debug: Checkpoint 1")

	pipe = rs.pipeline()
	configure = rs.config()
	print(f"Debug: Checkpoint 2")

	# #Added this to stop loading from bag files
	# PLAY_FROM_FILE=False
	# if PLAY_FROM_FILE:
	...
	while True:
		try:
			...
			print(f"Debug X:{x} y{y} w {w} h {h} and success {success}")				
			...
			info = [("Tracker", algo),("Success", "Yes" if success else "No"),("FPS", "{:.2f}".format(fps)),("Distance",calc_z)]
