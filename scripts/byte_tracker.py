...
class TrackerByte(Node):
    ...
    def check_leader(self,tracked_bboxs,ids):
        ...
        Bbox corners values   
        '''
        #Check if there are any person detected
        bbox=[]  
        lead_idx=None
        for index,id in enumerate(ids):
            if id==self.tracked_idx:
                lead_idx=index
            ## If leader is unknown and marker is available , this is 
        ...
        #Check if the index of the leader is still visible 
    ...
    def get_position(self,tracked_bbox):
        ...
        #Clipping tracker cooridinates outside the frame
    ...
    def imageflow_demo(self):          
        # if self.frame_id % 20 == 0:
            # self.get_logger().info(f"Processing frame {self.frame_id}, {1. / max(1e-5, self.timer.average_time)}")
        ...
        if (self.robot_stream_colour) is not None:
            ...
            if outputs[0] is not None:
                ...
                if len(tracked_bbox)>0:
                    ...
                    print_pos=str(tracked_position[2])+" m"
